...
![Map of FLOwPER observation points snapped to the NHDPlusHR hydrography dataset. The uppermost FLOwPER point snapped to a flowline based on a successful name match. The lowermost FLOwPER point did not snap to any flowline since its search buffer region did not contain any flowlines.\label{fig:gps_rmse_fig}](gps_rmse_fig.jpeg)
...
![Map of FLOwPER observation points snapped to a 2-m gridded TerrainWorks hydrography dataset, demonstrating the dynamic search buffer distances used based on the observations’ GPS horizontal accuracy.\label{fig:tw_fig}](tw_fig.jpeg)
